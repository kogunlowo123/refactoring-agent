"""Refactoring Agent - Main Agent Implementation."""
...
class RefactoringAgentAgent:
    ...
    def __init__(self):
        ...
        self.smell_detection_enabled = True
        self.safe_refactoring_enabled = True
        self.behavior_preservation_enabled = True
        self.test_verification_enabled = True
        self.rollback_support_enabled = True
        self._tool_dispatch = {
            "detect_code_smells": self.tools.detect_code_smells,
            "suggest_refactoring": self.tools.suggest_refactoring,
            "execute_refactoring": self.tools.execute_refactoring,
            "verify_behavior": self.tools.verify_behavior,
            "calculate_complexity": self.tools.calculate_complexity,
            "extract_method": self.tools.extract_method,
        }
        logger.info(
            "agent_initialized",
            ...
            features=['smell_detection', 'safe_refactoring', 'behavior_preservation', 'test_verification', 'rollback_support'],
        )
    ...
    async def process_message(self, request: ChatRequest, user_id: str) -> ChatResponse:
        ...
        rag_results = await self.rag_pipeline.retrieve(
            ...
            top_k=5,
            filters={"user_id": user_id, "domain": "software_engineering"},
        )
    ...
    async def _maybe_execute_tools(self, message: str, context: str) -> list[dict]:
        ...
        tool_keywords = {
            ...
            "execute_refactoring": ['execute', 'refactoring'],
            "verify_behavior": ['verify', 'behavior'],
            "calculate_complexity": ['calculate', 'complexity'],
